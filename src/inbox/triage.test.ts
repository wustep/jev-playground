...
import { REASON_IDS, type ReasonActivations, type ReasonSettings } from './types'
...
describe('scoreActions', () => {
  it('recommends Delete for obvious spam', () => {
    const result = scoreActions(activations({ spam: 0.99 }), DEFAULT_REASON_SETTINGS)
  ...
  it('recommends Leave for a real person with no other pull', () => {
    const result = scoreActions(activations({ person: 0.97 }), DEFAULT_REASON_SETTINGS)
    expect(result.recommended).toBe('Leave')
    expect(result.scores.Leave).toBeGreaterThan(0.6)
  })

  it('recommends Review for security mail', () => {
    const result = scoreActions(activations({ security: 0.97 }), DEFAULT_REASON_SETTINGS)
  ...
  it('gives every action its own 0–1 confidence and sums to 1', () => {
    const result = scoreActions(activations({ promotional: 0.8, person: 0.7 }), DEFAULT_REASON_SETTINGS)
  ...
  it('moves a mixed promo-from-a-friend toward Delete when promo is upweighted', () => {
    ...
    expect(promoHeavy.scores.Delete).toBeGreaterThan(defaultResult.scores.Delete)
  })
...
describe('seed inbox', () => {
  it('uses Gmail users.messages field names and flattened helpers', () => {
    ...
    expect(first.payload.parts?.some((p) => p.mimeType === 'text/plain')).toBe(true)
    expect(first.payload.parts?.some((p) => p.mimeType === 'text/html')).toBe(true)
  ...
  it('covers the demo genres', () => {
    ...
    expect(subjects.some((s) => /sign-in|security|dependabot/i.test(s))).toBe(true)
  })

  it('includes Priya’s forwarded sale as the mixed promo-from-a-friend case', () => {
    const priya = INBOX_SEED.find((entry) => entry.message.id === '18f0f9a0b1c2d3e4')
    expect(priya?.message.sender).toMatch(/Priya Raman/)
    expect(priya?.reasons.person).toBeGreaterThan(0.8)
    expect(priya?.reasons.promotional).toBeGreaterThan(0.5)
    if (!priya) return
    const result = triageItem(priya, DEFAULT_REASON_SETTINGS)
    expect(result.scores.Delete + result.scores.Review + result.scores.Leave).toBeCloseTo(1, 8)
  })
