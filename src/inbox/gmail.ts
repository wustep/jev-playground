...
function htmlWrap(subject: string, body: string): string {
  const escaped = body.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('\n', '<br>\n')
...
export function makeGmailMessage(draft: MessageDraft): GmailMessage {
  ...
  const headerBytes = headers.reduce((sum, item) => sum + item.name.length + item.value.length + 4, 0)
  ...
  return {
    ...
    snippet,
    historyId: draft.historyId ?? String(21_000_000 + Number.parseInt(draft.id.slice(-6), 16)),
    internalDate,
...
export function item(draft: MessageDraft, reasons: Partial<ReasonActivations>): InboxItem {
  return {
