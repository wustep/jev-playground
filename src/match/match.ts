...
const LOOKING_TO_HOBBIES: Record<LookingFor, Hobby[]> = {
  'weekend adventures': ['climbing', 'bouldering', 'trail running', 'cycling', 'sailing'],
...
function noteFor(id: MatchDimensionId, you: YouProfile, person: Person, score: number): string {
  if (id === 'hobbies') {
...
function hypothesis(you: YouProfile, person: Person, reasons: MatchReason[]): string {
  const top = reasons[0]
...
export function matchPerson(you: YouProfile, person: Person): MatchResult {
  const dims: { id: MatchDimensionId; score: number; weight: number }[] = [
    ...
    { id: 'lookingFor', score: jaccard(you.lookingFor, person.lookingFor), weight: 1.1 },
