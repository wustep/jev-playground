...
import { essentialTones, leadVoicing, sortAscending } from '../voiceLeading'

// ── minimal cells ───────────────────────────────────────────────────────────

/** Cell shapes as indices into the voicing (3 = the bottom note an octave up). */
const CELLS: Record<'plain' | 'additive' | 'inverted', number[]> = {
  plain: [0, 1, 2, 1],
  additive: [0, 1, 2, 3, 2, 1],
  inverted: [2, 1, 0, 1],
}
...
export function minimalCells(bar: BarContext): BarNotes {
  ...
  const low = nearestNote([bar.chord.root], bar.memory.bass ? midiOf(bar.memory.bass) : 43, 36, 50)
  bar.memory.bass = low
  const fifth = nearestNote([bar.chord.core[2]], midiOf(low) + 7)
...
const RIGHT_HAND: Record<'sixteen' | 'twelve', Record<BarRoleId, Hit[]>> = {
  sixteen: {
...
/** Left-hand groupings: 3+3+2 (+3+3+2) in 4/4; 3+3 vs 2+2+2 hemiola in 12-tick bars. */
const OSTINATO: Record<MeterId, number[][]> = {
  four_four: [[3, 3, 2, 3, 3, 2]],
...
export function syncopatedOstinato(bar: BarContext): BarNotes {
  ...
  const root = nearestNote([bar.chord.root], bar.memory.bass ? midiOf(bar.memory.bass) : 41, 34, 48)
  ...
  const grouping = groupings[bar.index % groupings.length]
  const left: Voice = []
  let tick = 0
...
const LUSH: Record<'sixteen' | 'twelve', Record<BarRoleId, LushShape>> = {
  sixteen: {
...
export function lushVoicings(bar: BarContext): BarNotes {
  ...
  bar.memory.bass = root
  const fifth = nearestNote([bar.chord.core[2]], midiOf(root) + 7)
  const seventh = bar.chord.core[3] ? nearestNote([bar.chord.core[3]], midiOf(root) + 10) : undefined
  const push = 6
  const left: Voice =
    role === 'cadence'
