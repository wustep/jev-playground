...
- **Allowlist, not a proxy.** The body must be one of eight typed ops (music: `concept`, `globals`, `bar`, `phrase`, `score`, and Debug-only `notes`; trolley: `trolley_cast`, `trolley_judge`), re-validated against the enums; the server builds the actual `state`/`questions`. Raw `state`/`questions` are rejected with 400. `bar` stays parseable for older tests; live planning uses `phrase`. The `notes` op is unchanged as a verb; optional `mode` (`guide`|`line`), `figure`, `goal`, `melodySoFar`, `bassSoFar`, `nextChord`, and `barIndex` are parsed against the closed note enums so a Coder allowlist that only lists `notes` still validates. Omitted `mode` = `line` (rhythm + `pitch_1..4` + `bass_pattern`). `mode: guide` asks `figure` + `goal` only. **`globals` now also asks `hookBars` (`2` \| `4` \| `8`)** — same fan-out as `phrasing`; Coder must allowlist that question id. No new op. Guide rest-slot silence and theme-return ornament are realize-only.
...
`renderPlan` (`src/render/renderPlan.ts`) is pure and deterministic. Nothing upstream of it knows about notes; nothing downstream knows about Jev. The plan JSON is shown in the UI and is **editable** — change `"texture"` or a bar's `"chord"`, hit *Apply to renderer*, and hear only the renderer's response to that one label. The **Notes** select sits next to **Planner** (code default). **Code writes the tune** is today’s `renderPlan` (default Generate). **Jev guides the tune** (`notes=guide`) is D1: Jev picks a closed figure + chord-tone goal; code writes the line; `melody_so_far` carries prior figures/goals. **Jev writes the line** (`notes=line`, legacy `notes=jev`) is today’s closed 4-slot rhythm+degrees path. Guide and Line need live Jev and turn Debug on so the notes exchanges show. Illegal RH falls back to `renderPlan`. The default Generate path is unchanged. Bars may be 4, 8, 16, 32, or 64; every form has a 64-bar layout (the 32-bar plan plus a second departure and return).
...
| Debug | `notes` × new plan bars | **line** (default / omitted `mode`): RH `rhythm` + `pitch_1..4` + `bass_pattern` in **one** POST. **guide**: `figure` + `goal` only (D1; no parallel degrees). | Closed enums only. Overlay writes the singing line over `renderPlan` accompaniment (LH stays). URL: `notes=code\|guide\|line` (`notes=jev` → `line`). Guide state carries `melody_so_far` as prior figures/goals; line state keeps rhythm + degree ids. Theme-return bars reuse source choices and re-spell. Not a new op. See [`docs/fable-context/NOTES_JEV_QUALITY.md`](docs/fable-context/NOTES_JEV_QUALITY.md). |
