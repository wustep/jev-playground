...
Same interface, no network, same order of decisions: privately, one of the style's variants (prelude, invention, chorale, sarabande, gigue, toccata for Bach), then globals from that variant's priors, then bars. Harmony is assembled from a per-style grammar — two-bar heads, travelling units and tails, four-bar phrases lifted from real pieces, codas — by the chosen form's phrase slots (the books also hold chord cycles and pedals, which no current form reaches), so eight bars come out of hundreds of combinations rather than three templates, and a returning `a` phrase really does return. A guard re-rolls anything whose roots spell I–V–vi–IV in any rotation. It's the demo default and a baseline to compare Jev's choices against. If a live Jev call fails, the UI says so and shows the stub's plan — it never passes a stub plan off as Jev's.
...
- **`accompaniment.ts`** has five patterns — `sustained`, `broken`, `pulse`, `stride`, `counterline` — each handed the melody's floor as a `ceiling` it may not cross. That one argument is most of what "the texture supports the tune" means, and it replaces asking twenty-three independent texture files to get it right separately. `counterline` is the one pattern that is not support: a two-voice invention has no accompaniment, it has a second tune of equal weight. It still sings in its own register under the first voice's floor, so the ceiling test covers all five.
- **`styleVoice.ts`** is style as the renderer sees it: one row per composer, four fields, none of which choose a note. Which notes to play is the plan's job; this is only how they are played — metric accent, articulation, touch, rubato.
