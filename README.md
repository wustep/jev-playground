...
| Debug | `notes` × new plan bars | RH `rhythm` + `pitch_1..4` + `bass_pattern` in **one** POST | Closed enums only. State includes `melody_so_far` (prior rhythm id + degree ids), `last_sounding_degree`, `bass_so_far`, this bar’s role/chord, and `next_chord` when known. Theme-return bars reuse source choices and re-spell. Not a new op. |
